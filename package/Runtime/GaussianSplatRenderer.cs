...
namespace GaussianSplatting.Runtime
{
    class GaussianSplatRenderSystem
    {
        ...
        public bool GatherSplatsForCamera(Camera cam)
        {
            ...
            {
                var gs = kvp.Key;
                if (gs == null || !gs.isActiveAndEnabled || !gs.HasValidAsset || !gs.HasValidRenderSetup || !gs.m_IsActiveFrame)
                    continue;
    ...
    {
        // 加在這裡！這用來控制 4DGS 播放
        public bool m_IsActiveFrame = true;
        ...
        {
            // 將我們之前加的 if (m_GpuPositions != null...) 那些全部刪除
        ...
        // 請將這段加在 GaussianSplatRenderer 類別的裡面、最底部
        public void UpdateAssetFor4D(GaussianSplatAsset newAsset)
        {
            if (m_Asset == newAsset) return;

            // 1. 關閉目前的渲染，釋放 GPU Buffer
            OnDisable();

            // 2. 抽換資料
            m_Asset = newAsset;

            // 3. 重新啟動渲染，分配新的 GPU Buffer
            OnEnable();
        }
    }
}